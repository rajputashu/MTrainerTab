...
import com.sisindia.ai.mtrainer.android.features.barcode.camera.CameraSizePair
...
object PreferenceUtils {
    ...
    fun getUserSpecifiedPreviewSize(context: Context): CameraSizePair? {
        return try {
            ...
            val sharedPreferences = PreferenceManager.getDefaultSharedPreferences(context)
            sharedPreferences.getString(previewSizePrefKey, null)?.let {
                CameraSizePair(
                    Size.parseSize(it),
                    sharedPreferences.getString(pictureSizePrefKey, null)?.let { string -> Size.parseSize(string) }
                )
            }
        } catch (e: Exception) {
            null
