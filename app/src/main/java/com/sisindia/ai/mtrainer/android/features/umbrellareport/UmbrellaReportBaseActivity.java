...
import com.sisindia.ai.mtrainer.android.databinding.ActivityUmbrellaReportBaseBinding;
import com.sisindia.ai.mtrainer.android.databinding.AdhocPostDialogBinding;
import com.sisindia.ai.mtrainer.android.features.syncadapter.UmbrellaSyncWorker;
...
public class UmbrellaReportBaseActivity extends MTrainerBaseActivity {
    private ActivityUmbrellaReportBaseBinding binding;
    private UmbrellaReportViewModel viewModel;
    public static final String BASE_IMAGE_PATH = Environment.getExternalStorageDirectory().getAbsolutePath() + "/Umbrella/";
        ...
        liveData.observe(this, message -> {
            switch (message.what) {
                ...
                case NavigationConstants.CLOSE_UMBRELLA_REPORT:
                    ...
                    WorkRequest immediateSync = new OneTimeWorkRequest.Builder(UmbrellaSyncWorker.class)
    ...
    protected int getLayoutResource() {
        return R.layout.activity_umbrella_report_base;
    }
    ...
    private void takeImage(String postName, int postId, int position) {
        try {
            ...
            String path = BASE_IMAGE_PATH + viewModel.dateFormat.format(new Date()) + "_" + Prefs.getInt(PrefsConstants.BASE_TRAINER_ID) + "_" + postId + ".jpg";
            //we will handle the returned data in onActivityResult
            captureIntent.putExtra(MediaStore.EXTRA_OUTPUT, getFileUri(path));
            viewModel.setIsLoading(false);
            Prefs.putStringOnMainThread(PrefsConstants.CURRENT_UMBRELLA_PICTURE_PATH, path);
