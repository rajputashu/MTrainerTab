...
public class StoragePath {
    ...
    /*public String[] getDeviceStoragesV2() {
        List<String> results = new ArrayList<>();
...
//        File[] externalDirs = context.getExternalFilesDirs(null);
        ...
        // If results are empty, return the internal storage path
        if (results.isEmpty()) {
            String internalStoragePath = Environment.getExternalStorageDirectory().getAbsolutePath();
            results.add(internalStoragePath);
        }

        // Filter results to retain only valid external storage paths
        filterExternalStoragePaths(results);

        // Convert List to String array
        return results.toArray(new String[0]);
    }*/

    /*private void filterExternalStoragePaths(List<String> results) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            for (int i = 0; i < results.size(); i++) {
                if (!results.get(i).toLowerCase().matches(".*[0-9a-f]{4}[-][0-9a-f]{4}")) {
                    Log.d("Tag", results.get(i) + " might not be extSDcard");
